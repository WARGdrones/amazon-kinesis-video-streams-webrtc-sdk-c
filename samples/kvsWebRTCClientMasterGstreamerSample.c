    ...
    switch (pSampleConfiguration->mediaType) {
        case SAMPLE_STREAMING_VIDEO_ONLY:
            if (pSampleConfiguration->useTestSrc) {
                ...
                gsconfig_env = getenv("GSTREAMER_WEBRTC_CONFIG");
                pipeline = gst_parse_launch(gsconfig_env, &error);
                //end Patrick added 
...
INT32 main(INT32 argc, CHAR* argv[])
{
    ...
    CHK_ERR((pChannelName = getenv("KVS_STREAM_NAME")) != NULL, STATUS_INVALID_OPERATION, "ENV Variable KVS_STREAM_NAME must be set");
